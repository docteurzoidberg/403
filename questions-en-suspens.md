# Questions en suspens

Liste des décisions à prendre, informations manquantes ou points à clarifier — toutes phases confondues.  
Une question résolue est cochée et reste en place (avec la réponse brève en italique sous la ligne si utile).

## Achat véhicule

- [ ] Prix final négocié avec le propriétaire (annonce : 3 250 €)
- [ ] Date prévisionnelle de prise en main / livraison
- [ ] Mode de récupération du véhicule depuis Varilhes (conduire vs faire transporter — cf. plan B "frais transport" dans `export`)
- [ ] Contrôle technique : qui le passe, quand, où (avant ou après prise en main ?)
- [ ] Montage des pneus évoqué au téléphone le 2026-05-24 — décisions prises (qui monte, quels pneus, où) ?
- [ ] Points à valider à la prise en main : état réel du freinage refait, embrayage, boîte, démarrage, corrosion châssis

## Préparation export (France → Maroc)

- [x] Atelier au Maroc : lequel ? Coordonnées, ville, contact
  - *El Hassan Tolerie, à Tiznit*
- [ ] Date cible de départ du véhicule vers le Maroc
- [ ] Modalités douanières / papiers d'export (carte grise, admission temporaire, etc.)
- [ ] Confirmer avec l'atelier la dispo des pièces "si disponible au Maroc" (capot supplémentaire conditionnel, manivelle de démarrage, bavettes)
- [ ] Hébergement sur place pendant la période atelier : où, combien de temps, budget
- [ ] Budget global préparation export (transport véhicule + ferry + hébergement)

## Restauration Maroc

- [ ] Snorkel : décision oui/non (conditionne l'achat d'un capot supplémentaire en France)
- [ ] Attelage sur mesure : décision oui/non (conditionne l'emplacement de l'anneau de remorquage arrière)
- [ ] Remplacement dynamo → alternateur : décision oui/non
- [ ] Couleur de peinture
- [ ] Choix tissu / matériau sellerie (ciel de toit, banquettes, garnitures de portes)
- [ ] Ski protection carter : matériau, épaisseur, design
- [ ] Fiabilisation ventilation radiateur : approche retenue (ventilateur additionnel, autre ?)

## Préparation raid (véhicule)

- [ ] CB radio : fournie par l'orga Crazy Dust ou à la charge du participant ?
- [ ] Si à notre charge : démarche ANRT (mail autorisation@anrt.ma) — à faire à quelle date ?
- [ ] Plaques de désensablage : fixation sur galerie ou support dédié (décision finale sur place)
- [ ] Modèle exact de batterie autonome EcoFlow (capacité, budget)

## Inscription Crazy Dust 2027

- [ ] Dates exactes du raid 2027
- [ ] Date d'ouverture des inscriptions
- [ ] Coût d'inscription (équipage + véhicule)
- [ ] Composition de l'équipage (co-équipier(s) confirmé(s) ?)
- [ ] Documents exigés par l'orga (assurance, passeport, permis, certificat médical ?)

## Sponsoring

- [ ] Décision : monter un dossier de sponsoring ou autofinancement complet ?
- [ ] Si oui : montant cible
- [ ] Si oui : cibles sponsors (marques pièces auto, locales, équipementiers raid, etc.)
- [ ] Si oui : contreparties proposées (logos véhicule, présence réseaux, reportage, etc.)
